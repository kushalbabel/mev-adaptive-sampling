...
class Timer():
    def __init__(self, transactions, involved_dexes, n_repeat=-1):
        ...
        if n_repeat > 1:
            with mp.Pool() as e:
                self.ctx_list = list(pool.starmap(simulate_efficient_hardhat.prepare_once, zip(repeat(ctx), repeat(self.transactions), range(n_repeat), repeat(self.involved_dexes))))
        else:
            self.ctx_list = [simulate_efficient_hardhat.prepare_once(ctx, self.transactions, -1, self.involved_dexes)]
     
    def evaluate(self, port_id):
        t0 = time.time()
        mev = simulate_efficient_hardhat.simulate(self.ctx_list[port_id], self.transactions, port_id, involved_dexes=self.involved_dexes)
    ...
    for p in paths:
        ...
        for _ in range(5):
            if n_repeat > 1:
                ...
                batch_output = [timer_obj.evaluate(port_id=-1)]
        ...
        assert abs(new_mevs[0] - orig_mevs[k]) < 1, f'new mev:{new_mevs[0]}, orig mev:{orig_mevs[k]}'
